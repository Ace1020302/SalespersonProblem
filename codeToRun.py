...
class codeToRun:
    ...
    def visualSeparator(self):
        print('===' * 100)
        print('===' * 100)
        print('===' * 100)

    def readNodes(self, fileName, skipFirstLine=True):
        arr = []
        ...
        for i in range(len(graph)):
            for j in range(len(graph)):
                ...
                if (i == j):
                    continue
                print(
                    f"Edge Weight to be added: From {labels[i]} to {labels[j]} --- {graph[i][j]}, {type(graph[i][j])}")
                g.add_edge(labels[i], labels[j], weight=graph[i][j])
        self.visualSeparator()
        self.printGraph(graph, labels)
        # for i in range(len(graph)):
        #     for j in range(len(graph)):
